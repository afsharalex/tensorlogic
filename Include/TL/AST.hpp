...
struct TensorRef {
  Identifier name;
  std::vector<IndexOrSlice>
      indices; // empty means scalar, can mix Index and Slice
  SourceLocation loc{};
};
...
struct Expr {
  SourceLocation loc{};
  std::variant<ExprTensorRef, ExprNumber, ExprString, ExprList, ExprParen,
               ExprCall, ExprBinary, ExprUnary>
      node;
};
...
struct DatalogAtom {
  ...
  // Terms: variable (lowercase Identifier), constant (StringLiteral for
  // Uppercase id/int/string), or arithmetic expression (ExprPtr)
...
struct GuardedClause {
  ...
  std::optional<ExprPtr>
      guard; // Optional guard condition (if present, acts as mask)
  SourceLocation loc{};
};
...
struct TensorEquation {
  ...
  std::string
      projection; // currently just "="; keep as text for future (+=, max=, ...)
  std::vector<GuardedClause>
      clauses; // Multiple guarded clauses (all contribute additively)
  SourceLocation loc{};
};
...
struct Query {
  ...
  // Additionally, for Datalog queries, we may allow a conjunction of atoms and
  // comparisons.
  std::variant<TensorRef, DatalogAtom> target;
  // If non-empty, represents a conjunctive query of atoms/conditions; the first
  // element is usually the same as `target` when `target` holds a DatalogAtom.
  std::vector<std::variant<DatalogAtom, DatalogNegation, DatalogCondition>>
      body;
...
struct DatalogFact {
  Identifier relation;
  std::vector<std::variant<StringLiteral, NumberLiteral>> constants;
  SourceLocation loc{};
};
...
using Statement = std::variant<TensorEquation, FileOperation, Query,
                               DatalogFact, DatalogRule, FixedPointLoop>;
