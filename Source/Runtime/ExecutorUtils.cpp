...
tryGatherNumericIndices(const TensorRef &ref, const Environment &env) {
...
std::vector<int64_t> resolveIndicesCreatingLabels(const TensorRef &ref,
                                                  Environment &env) {
  std::vector<int64_t> indices;
...
bool allConstants(const DatalogAtom &atom) {
  for (const auto &t : atom.terms) {
    if (std::holds_alternative<Identifier>(t)) {
      ...
      if (!id.name.empty() &&
          std::islower(static_cast<unsigned char>(id.name[0]))) {
        return false;
      }
    }
...
bool tryParseEinsumCall(const ExprPtr &expr, std::string &spec_out,
...
const ExprTensorRef *asExprTensorRef(const ExprPtr &ep) {
  ...
  const Expr *cur = e;
  while (true) {
    if (const auto *tr = std::get_if<ExprTensorRef>(&cur->node))
      return tr;
    if (const auto *par = std::get_if<ExprParen>(&cur->node)) {
      if (!par->inner)
        return nullptr;
...
static std::vector<int64_t> shapeFromRef(const TensorRef &ref) {
  ...
  for (const auto &ios : ref.indices) {
    if (std::holds_alternative<Index>(ios.value)) {
      ...
      // Slice - use default extent
      dims.push_back(kDefaultExtent);
    }
  }
  return dims;
}
...
static Tensor adaptTensorToRef(const Tensor &t, const TensorRef &ref) {
  ...
  if (have > static_cast<int64_t>(want)) {
    r = r.squeeze();
    if (r.dim() == static_cast<int64_t>(want))
      return r;
    return r;
  }
...
bool tryLowerIndexedProductToEinsum(const TensorRef &lhs, const ExprPtr &rhs,
                                    std::string &spec_out,
                                    std::vector<Tensor> &inputs_out,
                                    Environment &env) {
  if (!rhs)
  ...
  if (!bin || bin->op != ExprBinary::Op::Mul)
    return false;
  ...
  // Allow numeric indices - they will be handled by valueForRef() which
  // properly slices tensors This enables patterns like W[i,j] State[j,0] where
  // 0 is a bound index

  auto collectNames = [](const TensorRef &r) {
    std::vector<std::string> names;
    names.reserve(r.indices.size());
    for (const auto &ios : r.indices) {
      if (std::holds_alternative<Index>(ios.value)) {
        const auto &idx = std::get<Index>(ios.value);
  ...
  // Use valueForRef() instead of env.lookup() to properly handle mixed
  // bound/free indices For example, State[j,0] will be correctly sliced to
  // State[:,0] before einsum

  // For left operand: create placeholder for base tensor if needed, then slice
  const std::string leftName = leftRef->ref.name.name;
  if (!env.has(leftName)) {
    // Create a TensorRef with only free variable indices for placeholder shape
    // inference
    TensorRef baseRef = leftRef->ref;
    // Remove numeric indices - keep only identifiers for shape inference
    std::vector<IndexOrSlice> freeIndices;
    for (const auto &ios : baseRef.indices) {
      if (std::holds_alternative<Index>(ios.value)) {
        const auto &idx = std::get<Index>(ios.value);
...
torch::indexing::Slice convertSlice(const tl::Slice& slice) {
  ...
  // Handle different slice combinations
  // PyTorch Slice constructor: Slice(start, end, step) where nullopt = None

  if (!slice.start.has_value() && !slice.end.has_value() && !slice.step.has_value()) {
    // Full slice ":"
    return Slice();
  }
  ...
  std::optional<int64_t> step_val;

  if (slice.start.has_value()) {
    try {
      start_val = std::stoll(slice.start->text);
    } catch (...) {
      start_val = 0;
    }
  }
  ...
  if (slice.end.has_value()) {
    try {
      end_val = std::stoll(slice.end->text);
    } catch (...) {
      end_val = std::nullopt;  // Invalid end -> treat as None
    }
  }

  if (slice.step.has_value()) {
    try {
      step_val = std::stoll(slice.step->text);
      if (step_val.value() == 0) {
        step_val = 1;  // Invalid step -> default to 1
      }
    } catch (...) {
      step_val = 1;
    }
  ...
  // Construct PyTorch slice with explicit values
  return Slice(start_val, end_val, step_val);
}
